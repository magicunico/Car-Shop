<template>
    <div>
        <b-table striped :items="warehouses" :fields="fields" :hover="true" ref="table" id="warehouses-list-table" v-if="!edit">
             <template slot="actions" slot-scope="data">
                 <span style="padding-left:20px;"><img src="../../assets/delete.svg" @click="deleteWarehouse(data.item.id)"></span>
                 <span><img src="../../assets/edit.svg" @click="editWarehouse(data.item.id)"></span>
             </template>
         </b-table>
        <b-form  v-else @submit.prevent="updateWarehouse()">
...
export default {
    ...
    methods:{
        ...
        editWarehouse(data){
        this.edit = true;
        axios.get("http://localhost:8080/warehouse/"+data)
        .then( result => {
            this.name = result.data.name;
            this.id=result.data.id;
        })
      },
        updateWarehouse(){let body = {
                'name' : this.name,
                'status' : '1',
                'id': this.id
            }

            axios.post("http://localhost:8080/warehouse/update",body);
            }
    },
    beforeMount(){
