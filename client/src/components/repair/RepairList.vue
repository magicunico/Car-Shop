...
export default {
  ...
  methods: {
    deleteRepair(data) {
      ...
      axios
        .delete(process.env.API_URL + "/repair/delete/" + data, config)
        .then(() => {
          this.$router.go();
        });
    },
    ...
    updateRepair() {
      ...
      axios.put(process.env.API_URL + "/repair/update", body, config);
    }
  },
  beforeMount() {
    axios
      .get(process.env.API_URL + "/repair/active")
